...
class AnuncianteBeanTest {
    ...
    void excecoesRemocaoAnuncio() {
        assertAll("Remove itens da lista de anuncios",
                () -> {
                    anunciante.getAnuncios().clear();
                    Exception exception = assertThrows(Exception.class, () ->
                            anunciante.removeAnuncio(0));
                    assertEquals("Não existem items para remover na lista", exception.getMessage());
                },
                () -> {
                    anunciante.getAnuncios().clear();
                    Exception exception = assertThrows(Exception.class, () ->
                            anunciante.removeAnuncio(-1));
                    assertEquals("Índice inválido! O índice precisa ser maior ou igual a 0", exception.getMessage());
                },
                () -> {
                    anunciante.getAnuncios().clear();
                    anunciante.addAnuncio(anuncio1);
                    ...
                    assertEquals("Índice inválido! O índice precisa ser menor que o tamanho da lista", exception.getMessage());
                });
    }
    ...
    void valorMedioAnuncios() {
        ...
        assertEquals(1033.33, anunciante.valorMedioAnuncios());
    }
}